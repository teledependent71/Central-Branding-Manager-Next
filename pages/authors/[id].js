...
import authorsPageInitialPathsF2913Resource from '../../resources/authors-page-initial-paths-f2913'
import authorsPageInitialProps72ceeResource from '../../resources/authors-page-initial-props-72cee'
  ...
  return (
    <>
      <div className="authors11-container">
        <Head>
          <title>Authors1 - Central Branding Manager</title>
          <meta
            property="og:title"
            content="Authors1 - Central Branding Manager"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPathsF2913Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialProps72ceeResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
